...
class CSV_read(file_read):
    __slots__ = ("_dtype", "_data", "_row_pos", "_chunk_size")

    def __init__(self, path, filename, stream_type, chunk_size):
        ...
        self._chunk_size = chunk_size #the chunk size of the data source
    ...
    def start(self): #return generator to perform the stream of data
        while True:
            if self._row_pos >=0 and self._row_pos < len(self._data):
                yield self._data[self._row_pos: self._row_pos+self._chunk_size]
                self._row_pos += self._chunk_size
            else:
                self._row_pos = 0
                return
...
class stream_collect:
    __slots__ = ("_path", "_files", "_name_pattern", "_device", "_chunk_size")
    ...
    def __init__(self, path, device):
        ...
        self._device = device
        self._chunk_size = self._device.PARAMS["streams"]["chunk_size"]
    ...
    def read_stream(self): #read group of files using CSV_read and JSON_read
        ...
        csv_data, json_data = {}, {}
        for i in self._chunk_size.keys():
            for j in csv_files:
                if i in j.name:
                    csv_data[i] = CSV_read(j.parent, j.name, i, self._chunk_size[i])
            for k in json_files:
                if i in k.name:
